...
// Intervalo y sincronización de tiempo
const long INTERVALO_MS = 5 * 60 * 1000UL; // 5 minutos (300000 milisegundos)
unsigned long proximoEnvio = 0;
// --- FUNCIÓN DE CONFIGURACIÓN ---
void setup() {
Serial.begin(115200);
while (!Serial);
Serial.println("\n--- INICIANDO SISTEMA CON TIMESTAMP UNIX Y SD CARD (SINCRONIZADO A 5 MIN) ---");
// Iniciar el RTC
if (! rtc.begin()) {
...
Serial.println("Sincronizando con el reloj para el próximo intervalo de 5 minutos...");
DateTime ahora = rtc.now();
// Calculamos cuántos minutos faltan para el próximo múltiplo de 5
int minutosActuales = ahora.minute();
int minutosParaSiguienteIntervalo = 5 - (minutosActuales % 5);
// Calculamos los segundos restantes en el minuto actual
int segundosActuales = ahora.second();
int segundosRestantes = 60 - segundosActuales;
// Calculamos el retraso total en milisegundos
// (minutos completos restantes * 60000) + (segundos restantes * 1000)
unsigned long retrasoInicialMs = (minutosParaSiguienteIntervalo - 1) * 60000UL + segundosRestantes * 1000UL;
proximoEnvio = millis() + retrasoInicialMs;
Serial.print("Hora actual: ");
char buffer[20];
sprintf(buffer, "%02d:%02d:%02d", ahora.hour(), ahora.minute(), ahora.second());
Serial.println(buffer);
Serial.print("Próxima lectura programada en: ");
Serial.print(retrasoInicialMs / 1000.0);
...
// Programamos el siguiente envío exactamente 5 minutos después del anterior
proximoEnvio += INTERVALO_MS;
}
}
